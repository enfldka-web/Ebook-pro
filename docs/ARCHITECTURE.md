...
# Milestone 3 추가분 — Sales Page Studio v1
...
<script src="js/thumbnail-studio.js"></script>
<script src="js/thumbnail-studio-io.js"></script>
<script src="js/thumbnail-intelligence.js"></script>
<script src="js/sales-page-studio.js"></script>      ← 신규
<script src="js/sales-page-studio-io.js"></script>    ← 신규, 항상 마지막
```

## 네임스페이스와 DOM 접두사

- `window.SalesPageStudio` — 상태 초기화, 화면 진입/복귀, 섹션 데이터, On/Off, 순서 변경, 선택, 텍스트
  편집, 템플릿/색상/레이아웃, 전체 render/부분 Preview render를 전부 이 객체 아래에 둠.
- 신규 DOM ID/클래스는 전부 `sps-` 접두사(`#sps-root`, `#sps-preview-list`, `.sps-structure-row`,
  `.sps-card`, `.sps-theme-chip` 등) — 기존 `sp-*`(크몽 상세페이지 카드뉴스), `ts-`(Thumbnail Studio),
  `ti-`(Thumbnail Intelligence) 접두사와 겹치지 않도록 의도적으로 분리.
- 신규 화면: `#cv-salespagestudio-state`(루트 `#sps-root`), 진입 버튼은 `#cv-result-state`에 "📄 Sales
  Page Studio" 1개만 추가.

## 기존 코드와의 접점 (읽기만 함, 수정 없음)

- 최초 진입 시 `APP.ebook`, `APP.ebook.sales`, `APP.ebook.chapters`를 읽어 8개 섹션의 초기 문구로
  안전하게 매핑합니다(`sales-page-studio-io.js`의 `buildSectionsFromEbook`, 순수 함수). 매핑 후에는
  `APP.ebook`/`APP.ebook.sales`를 다시 쓰지 않습니다.
- `atlasSetWorkspaceStage('sales', ...)` — 기존 워크스페이스 진행 표시와 동일하게 재사용(Thumbnail
  Studio와 동일한 패턴).
- 개별 PNG Export는 기존 `dlSpSlide`와 동일한 `html2canvas(el,{scale:2,useCORS:true,allowTaint:true})`
  파라미터를 그대로 재사용해 **1080×1350px**을 만듭니다(기존 코드의 실제 출력 규격을 그대로 준용,
  임의 지정 아님). ZIP 묶음은 이미 `index.html`에 로드되어 있는 JSZip(기존에는 `downloadDocx`에서만
  사용)을 재사용한 신규 구현입니다.
- `renderCvSalesPage`, `showSalesThemeModal`, `checkAndShowSales`, `showCvSales`, `hideCvSales`,
  `dlSpSlide`, `dlAllSlides`, `downloadSelectedSpSlides`, `downloadKmongLongPage`,
  `renderKmongThumbnails`, `renderKmongListing`, `bootstrap.js`의 몽키패치, `APP.ebook.sales` 스키마,
  Thumbnail Studio/Intelligence — 전부 한 글자도 수정하지 않았습니다.

## 상태 구조 — 분석 보고서 대비 조정 사항

사전 분석 보고서의 제안 상태 구조를 그대로 따르되, `templateId`와 `colorId`는 v1에서 완전히 동일한
역할(6가지 색상 테마 선택)이라 **`themeId` 하나로 통합**했습니다. 템플릿(전체 디자인)과 카드 레이아웃
(섹션별 정보 배치)은 서로 다른 개념으로 유지되며, 카드 레이아웃은 섹션마다 독립적인 `layoutId` 필드로
남아 있습니다.

```js
APP.salesPageStudio = {
  version: 1,
  sourceEbookKey: '',      // 제목|부제목|챕터수 로 결정적 생성, 전자책이 바뀌면 재초기화 트리거
  themeId: 'navy-gold',    // 6종 색상 테마 (templateId 통합)
  selectedSectionId: 'hero-1',
  sections: [
    { id, type, enabled, order, title, body, badge, cta, layoutId }
    // beforeAfter 섹션만 body 대신 beforeText/afterText 두 개의 텍스트 필드를 추가로 가짐
  ]
};
```

## 저장/불러오기 — `atlasCollectDraft`/`atlasLoadDraft` 최소 훅

Thumbnail Studio의 `thumbnailStudio` 필드와 완전히 동일한 패턴으로 `salesPageStudio` 필드 1개만
추가했습니다(`js/application.js`, 총 3줄 추가). 구버전 드래프트(필드 없음)를 불러오면 `SalesPageStudio.init()`이
현재 `APP.ebook` 데이터로 안전한 기본 상태를 새로 만듭니다.

## 건드리지 않은 것

`js/bootstrap.js`, 기존 렌더러의 상세페이지/썸네일 함수, 기존 Prompt Engine, 기존 `APP.ebook.sales`
저장 스키마, Thumbnail Studio/Intelligence — 전부 무수정입니다.
