...
function newEbookProgressState(){
  return {
    ...
    appendices:null, appendicesStatus:'pending', appendixItemStatus:new Array(3).fill('pending'),
...
function renderEbookProgressUI(){
  ...
  if(list){
    ...
    /* 2026-09-10: 부록도 review와 동일하게 내부적으로 3개(부록별 1개씩) 호출로
       나뉘어 진행되므로(아래 continueEbookPipeline 참고), "생성 중..."만 보이면
       그 사이 몇 분간 변화가 없어 멈춘 것처럼 보일 수 있다 — 몇 개 중 몇 개가
       끝났는지 함께 보여준다. */
    var appendixDoneCount=(p.appendixItemStatus||[]).filter(function(s){return s==='completed';}).length;
    var aLabel='부록'+(p.appendicesStatus==='completed'?' 완료':p.appendicesStatus==='processing'?' 생성 중... ('+appendixDoneCount+'/3)':p.appendicesStatus==='failed'?' 실패':' 대기');
...
async function continueEbookPipeline(){
  ...
  try{
    ...
    /* 2026-09-10 버그 수정 — 부록 3개(체크리스트/도구비교/실행플랜)를 한 번의
       호출로 다 받으려 하면 합쳐서 max_tokens(16000)를 넘기기 쉬워 응답이
       중간에 잘리고, 자동 재시도 3회도 매번 같은 크기 문제로 똑같이 실패해
       "부록 실패"로 멈췄다(review의 90%-멈춤 버그와 동일한 원인). 챕터/review와
       같은 방식으로 부록도 1개씩 3개의 개별 호출로 쪼갠다 — 완료된 부록은
       재개 시 건너뛴다(§1과 동일). 레거시로 저장된 진행 상태(이 필드가
       생기기 전에 저장된 것)에도 안전하게 기본값을 채운다. */
    if(p.appendicesStatus!=='completed'){
      p.status='appendices';
      if(!p.appendixItemStatus)p.appendixItemStatus=new Array(3).fill('pending');
      if(!Array.isArray(p.appendices))p.appendices=[];
      p.appendicesStatus='processing';renderEbookProgressUI();

      for(var ai=0;ai<3;ai++){
        if(p.appendixItemStatus[ai]==='completed')continue;
        if(p.stopRequested){ p.status='stopped'; persistEbookProgress(); renderEbookProgressUI(); return; }
        p.appendixItemStatus[ai]='processing';renderEbookProgressUI();
        var appendix=await E.generateAppendix(p.outline, ai, p.market);
        p.appendices[ai]=appendix;
        p.appendixItemStatus[ai]='completed';
        persistEbookProgress();
        renderEbookProgressUI();
      }
    ...
    if(!p.outline){
      ...
      else if(p.appendicesStatus==='processing'){
        p.appendicesStatus='failed'; p.failedUnitId='appendices';
        var apIdx=(p.appendixItemStatus||[]).indexOf('processing');
        if(apIdx!==-1)p.appendixItemStatus[apIdx]='failed';
      }
...
function atlasDownloadEbookPdf(){
  ...
  /* 2026-08-14: 사용자 지시 — "PDF 페이지 수가 워드 페이지 수와 같아야 한다".
     ...
     2026-08-20: 한때 "페이지를 나누지 않고 A4 한 장에 비율을 줄여서
     맞추는" 방식으로 바꿨다가(당시 PR) 사용자가 다시 "내용이 한 장보다
     길면 페이지를 추가하는 게 맞다, 비율을 줄이지 마라"고 명시적으로
     되돌려 이 방식(여러 장으로 나누기)으로 복원했다 — 다음에 이 부분을
     다시 "한 장에 맞추기"로 바꾸고 싶어질 수 있으니, 이미 시도했다가
     명시적으로 거부된 방향임을 기록해둔다. */
...
function atlasBuildThumbnailPrompt(ebook){
  ...
  lines.push('3. 권위·신뢰 전략: 절제된 미니멀 구도와 고급스러운 톤으로 "이 책을 만든 사람은 전문가"라는 신뢰감을 형성합니다.');
  ...
  lines.push('');
  lines.push('[디자인 방향]');
  /* 2026-08-21: 사용자 리포트 — 서로 다른 책으로 썸네일을 여러 번 만들어봐도
     배경색·글씨색·구조가 계속 비슷하게 나온다는 지적. 원인으로 보이는 두
     지점을 고쳤다: (1) 전략 3번 설명에 "다크 네이비+골드, 딥블루"라는 구체
     색상 예시가 박혀 있어서, 5개 전략 중 유일하게 색까지 못박힌 이 전략이
     외부 AI 입장에서 가장 "안전하고 구체적인" 선택지로 보여 반복 선택되기
     쉬웠다(전략 설명에서 예시 색상 삭제). (2) 아래 디자인 방향에 "자기계발/
     비즈니스는 다크 톤"이라는 분야→색 고정 매핑이 있었는데, Atlas에서 만드는
     책 대부분이 이 분야에 속하다 보니 사실상 항상 같은 톤으로 귀결됐다 —
     고정 매핑 대신 실제로 서로 다른 팔레트 후보 목록을 주고, "가장 무난한
     톤으로 도망치지 말라"고 명시적으로 지시한다. 구도(제목 위치·오브제
     배치)도 기존엔 "상단 또는 중앙" 두 가지뿐이라 매번 비슷했으므로 배치
     패턴 자체를 다양화했다. */
  lines.push('- 아래 색감 팔레트 후보 중, 위에서 고른 전략과 이 책의 실제 분위기·분야에 가장 잘 어울리는 것 하나를 참고해 이 책만의 조합으로 조정하세요(그대로 베끼지 마세요): ① 다크 네이비+골드 ② 딥 포레스트 그린+브라스 ③ 테라코타+크림 ④ 세이지 그린+차콜 ⑤ 비비드 코랄 그라디언트 ⑥ 소프트 파스텔 그라디언트 ⑦ 모노크롬+단일 포인트 컬러 ⑧ 어스 톤(황토·베이지·브라운) ⑨ 일렉트릭 블루+화이트. "자기계발/비즈니스는 다크 네이비+골드"처럼 분야마다 하나의 뻔한 팔레트로 자동 수렴하지 말고, 같은 분야의 책이라도 제목·부제의 실제 뉘앙스에 따라 다른 팔레트를 선택하세요 — 이 프롬프트는 서로 다른 여러 책 표지를 만드는 데 반복해서 쓰이므로, 매번 가장 무난하고 안전해 보이는 톤(특히 다크 네이비+골드)으로 도망치지 않는 것이 중요합니다.');
  lines.push('- 배경은 과하게 복잡하지 않게, 여백을 살린 구도로 만들어주세요.');
  lines.push('- 이 책의 핵심 주제를 상징하는 시각적 오브제(사물·아이콘·일러스트) 하나를 배치해 "무슨 책인지"가 이미지만 봐도 느껴지게 해주세요. 오브제 위치와 배경 구성 방식도 아래처럼 다양하게 열어두고 이 책에 가장 잘 맞는 것을 고르세요 — 항상 같은 구도로 고정하지 마세요: 제목 상단+오브제 중앙, 제목 중앙+오브제가 배경 전체를 채움, 제목 하단 밴드+오브제 상단, 좌우 분할(한쪽 텍스트/다른 한쪽 오브제) 등.');
  ...
  if(market==='global'){
    lines.push('- 위 제목(영문)을 굵고 가독성 높은 산세리프 서체로, 위에서 정한 구도에 맞는 위치에 2~3줄 이내로 배치해주세요.');
  }else{
    lines.push('- 한글 타이포그래피를 정확하게 렌더링할 수 있다면, 위 제목을 굵고 가독성 높은 산세리프 서체로 위에서 정한 구도에 맞는 위치에 2~3줄 이내로 배치해주세요.');
    lines.push('- 한글 텍스트 렌더링이 정확하지 않다면 제목 텍스트는 빼고, 대신 그 위치에 텍스트를 나중에 얹을 수 있는 여백 공간을 의도적으로 남겨주세요.');
  }
  lines.push('');
  lines.push('[규격]');
...
/* 2026-08-14: 사용자 지시로 세 번째로 방향을 바꿨다 — (1차) 카피 작성 텍스트
   ...
   2026-08-20: 사용자가 실제 생성 결과(스크린샷)로 지적한 문제 두 가지를
   반영해 네 번째로 방향을 바꿨다 — (1) "텍스트를 렌더링하지 마세요" 지시
   때문에 FAQ 같은 단계가 아무 정보 없는 빈 카드로 나왔다 → 이제 1단계에서
   실제 카피 문구를 직접 쓰게 하고, 2단계에서 그 문구를 이미지 안에 진짜
   텍스트로 렌더링하도록 뒤집었다. (2) 9장을 한 번에 3x3 콜라주 한 장으로
   합쳐 반환해 개별 다운로드가 안 됐다(히어로/마지막 CTA도 구분 없이 같은
   그림으로 나옴) → "9장을 각각 독립된 파일로, 콜라주로 합치지 말라"는
   지시와 "1번과 9번은 서로 달라야 한다"는 지시를 명시적으로 추가했다.
   Atlas 자신은 여전히 이미지를 생성하지 않는다(그 요청을 받는 건 사용자가
   붙여넣는 외부 AI 도구) — 이 원칙은 그대로 유지된다. */
function atlasBuildSalesPagePrompt(ebook){
  ...
  lines.push('9장은 반드시 각각 독립된 개별 이미지 파일로 하나씩 생성해주세요 — 9칸으로 나눈 콜라주/그리드 이미지 한 장으로 합치지 마세요. 각 이미지를 대화 안에서 따로 클릭해 개별적으로 저장(다운로드)할 수 있어야 합니다.');
  lines.push('※ 이 프롬프트를 붙여넣을 때, 앞서 STEP 4에서 만든 표지(썸네일) 이미지를 참고 이미지로 함께 첨부해주세요 — 표지와 같은 색감·톤·아트 스타일로 9장이 이어지게 만들어달라는 뜻입니다. 표지 이미지를 아직 안 만들었다면 이 항목은 건너뛰어도 됩니다.');
  lines.push('');
  lines.push('[책 정보]');
  lines.push('- 제목: '+title);
  if(subtitle)lines.push('- 부제: '+subtitle);
  ...
  lines.push('');
  lines.push('[작업 순서]');
  lines.push('1단계: 위 정보를 근거로 아래 9단계 판매 스토리에 맞는 카피 문구를 단계별로 하나씩 직접 쓰세요. 이 흐름은 "주의를 끌고 → 문제에 공감시키고 → 위기감을 키우고 → 해결책으로 안심시키고 → 갖고 싶게 만들고 → 신뢰를 굳히고 → 변화를 증명하고 → 남은 의심을 없애고 → 지금 사게 만드는" 검증된 구매 전환 구조입니다. 각 단계 문구는 짧고 강렬한 한두 문장(헤드라인 + 필요하면 짧은 서브카피)으로 쓰세요 — 이 문구를 2단계에서 실제로 이미지 안에 텍스트로 렌더링합니다.');
  lines.push('2단계: 그 흐름에 맞춰, 아래 9단계 순서 그대로 이미지를 하나씩 실제로 생성해서 보여주세요(설명이 아니라 진짜 이미지). 1단계에서 쓴 그 단계의 카피 문구를 실제 상세페이지처럼 이미지 안에 크고 읽기 쉬운 글자로 렌더링하세요(오탈자 없이 정확하게, 문구는 짧게 유지해 잘리지 않게). 각 단계에 적힌 "목적"은 그 이미지가 독자의 마음속에서 반드시 해내야 하는 역할입니다:');
  ...
  lines.push('- 9장 전체가 같은 색감·톤·아트 스타일로 이어지는 하나의 세트처럼 보이게 통일해주세요(첨부한 표지 이미지가 있다면 그 톤을 그대로 이어가세요).');
  lines.push('- 비슷한 주제의 다른 전자책 상세페이지에서 흔히 보이는 뻔한 스톡 이미지·클리셰 구도는 피하고, 이 책의 실제 소재와 타겟 독자를 반영한 고유한 비주얼을 만들어주세요.');
  lines.push('- 9장은 서로 뚜렷하게 다른 장면·구도여야 합니다 — 특히 1번(히어로)과 9번(마지막 CTA)은 같은 목적이 아니므로 같은 그림처럼 보이면 안 됩니다.');
  lines.push('- 사람 얼굴, 실존 인물, 저작권이 있는 캐릭터·로고·브랜드는 넣지 마세요.');
  lines.push('- 1단계에서 쓴 카피 문구를 각 이미지 안에 실제 텍스트로 렌더링하세요 — 실제 상세페이지 디자인처럼 헤드라인은 크고 굵게, 서브카피는 작게 배치합니다. 텍스트가 흐릿하거나 잘리거나 철자가 틀리면 안 됩니다 — 정확히 읽히지 않을 것 같으면 문구를 더 짧게 줄이세요.');
...
function atlasBuildListingMaterialsPrompt(ebook){
  ...
  lines.push('- 구매(결제) 직후 구매자에게 보여줄 안내 문구입니다. 분량은 300자에 가깝게(280~300자) 작성해주세요.');
  lines.push('- 파일이 어떤 경로(채팅/자동다운로드 등)로 전달되는지는 플랫폼마다 달라 구체적으로 지어내지 마세요 — "자료를 꼭 확인해주세요" 정도로만 안내합니다.');
  lines.push('- 다음 순서와 어조로 작성해주세요: ① 진심 어린 감사 인사 ② 준비 과정에 대한 솔직한 소감(정성껏 고민하며 만들었다는 진심) ③ 궁금한 점이나 막히는 부분은 언제든 편하게 문의해달라는 안내(AS에 대한 신뢰감) ④ 구매 확정 후 남겨주는 후기 한 줄이 다음 자료를 더 정성껏 만드는 진짜 원동력이 된다는 진솔한 요청 ⑤ 마무리 감사 인사.');
  lines.push('- 과장이나 홍보 문구 없이, 담백하고 진솔한 톤으로 작성해주세요.');
  lines.push('');
  lines.push('3. 검색 키워드 5가지');
  lines.push('- 이 책을 검색으로 찾을 만한 사람들이 네이버·구글·플랫폼 검색창에 실제로 입력할 법한 키워드 5개를 나열해주세요.');
  lines.push('- 책 제목이나 문장을 그대로 옮기지 말고, 사람들이 실제로 검색할 때 쓰는 짧은 단어·구 형태(2~5단어 내외)로 작성해주세요. 너무 포괄적인 한 단어(예: "부업")보다 실제 검색 의도가 드러나는 구체적인 조합(예: "퇴근 후 부업 추천")을 우선하세요.');
  lines.push('- 5개는 서로 다른 검색 의도(주제 키워드, 타겟 독자 키워드, 문제/고민 키워드, 방법/노하우 키워드, 롱테일 키워드 등)를 하나씩 대표하도록 다양하게 구성해주세요.');
  lines.push('');
  lines.push('4. 자주 묻는 질문 Q&A 5가지');
  lines.push('- 구매를 망설이는 사람이 실제로 궁금해할 질문과 답변 5쌍을 작성해주세요. 답변은 255자 내외로 작성해주세요.');
  lines.push('');
...
function resumeIncrementalEbookGeneration(){
  ...
  if(p.appendicesStatus==='failed'){
    p.unitRetryCount.appendices=(p.unitRetryCount.appendices||0)+1; p.appendicesStatus='pending';
    // 부록 내부의 3개 단위 중 실패한 것만 pending으로 되돌린다(완료된 것은 그대로 유지 — §1과 동일)
    if(p.appendixItemStatus)p.appendixItemStatus=p.appendixItemStatus.map(function(s){return s==='failed'?'pending':s;});
  }
