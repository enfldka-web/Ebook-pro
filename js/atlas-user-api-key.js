/* js/atlas-user-api-key.js — V3 Phase 2 Round 32 (2026-08-07)
   ...
   각 사용자가 자신의 Anthropic API 키로 직접 낸다. 이 키는 오직 이
   브라우저의 localStorage에만 저장되고, Atlas 서버(있다면)나 다른 어떤 곳으로도
   전송되지 않는다 — js/anthropic-gateway-client.js가 이 키가 있으면 그 즉시
   api.anthropic.com을 브라우저에서 직접 호출하고, 없으면 기존처럼 Atlas Node
   Gateway(로컬 서버, 운영자 본인 테스트용)를 거친다 — 기존 로컬 게이트웨이
   경로는 전혀 삭제하지 않았다(회귀 없음, Never-Delete 원칙).
   2026-08-10: 썸네일/상세페이지(및 그 이미지 생성용 OpenAI 키)가 전면
   삭제되어 이제 Anthropic 키 한 종류만 남는다. */
